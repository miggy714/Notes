...
% Lines: log10(E) = 2*log10(rho) + C  =>  E = 10^C * rho^2
\pgfplotsinvokeforeach{-1,0,1,2,3}{
  ...
  \addlegendentry{$\log_{10}E=2\log_{10}\rho + #1$}
}
...
\\
\begin{align*}
 \text{Bending stiffness per mass of material} &= \frac{EI}{m}, \quad m = \rho V \\
   &= \frac{EI}{\rho V} \\
   \text{Material index } M &= \frac{E}{\rho} \\
   \log(M) &= \log(E) - \log(\rho) \\
   \text{Alternative loci: } \log(E) &= 2\log(\rho) + \log(\text{const})
\end{align*}
